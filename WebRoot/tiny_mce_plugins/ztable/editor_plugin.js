        ...
        createControl: function(n, cm) {
            ...
            if (n == 'ztablecontrols') {
                    ...
                    c.onRenderMenu.add(function(c, m) {
                        ...
                        m.addSeparator({id : DOM.uniqueId() });
                        c.ztmenu['row_props'] = m.add({id : DOM.uniqueId(), title : 'table.row_desc', cmd : 'mceTableRowProps', ui: true, icon: 'row_props'});
                        c.ztmenu['row_props'].setDisabled(1);
                        c.ztmenu['cell_props'] = m.add({id : DOM.uniqueId(),title : 'table.cell_desc', cmd : 'mceTableCellProps', ui: true, icon: 'cell_props'});
                        c.ztmenu['cell_props'].setDisabled(1);
                        m.addSeparator({id : DOM.uniqueId() });
                        c.ztmenu['row_before'] = m.add({id : DOM.uniqueId(), title : 'table.row_before_desc', cmd : 'mceTableInsertRowBefore', icon: 'row_before'});
                        c.ztmenu['row_before'].setDisabled(1);
                        c.ztmenu['row_after'] = m.add({id: DOM.uniqueId(), title : 'table.row_after_desc', cmd : 'mceTableInsertRowAfter', icon: 'row_after'});
                        c.ztmenu['row_after'].setDisabled(1);
                        c.ztmenu['delete_row'] = m.add({id: DOM.uniqueId(), title : 'table.delete_row_desc', cmd : 'mceTableDeleteRow', icon: 'delete_row'});
                        c.ztmenu['delete_row'].setDisabled(1);
                        m.addSeparator({id : DOM.uniqueId() });
                        c.ztmenu['col_before'] = m.add({id: DOM.uniqueId(), title : 'table.col_before_desc', cmd : 'mceTableInsertColBefore', icon: 'col_before'});
                        c.ztmenu['col_before'].setDisabled(1);
                        c.ztmenu['col_after'] = m.add({id: DOM.uniqueId(), title : 'table.col_after_desc', cmd : 'mceTableInsertColAfter', icon: 'col_after'});
                        c.ztmenu['col_after'].setDisabled(1);
                        c.ztmenu['delete_col'] = m.add({id: DOM.uniqueId(), title : 'table.delete_col_desc', cmd : 'mceTableDeleteCol', icon: 'delete_col'});
                        c.ztmenu['delete_col'].setDisabled(1);
                        m.addSeparator({id : DOM.uniqueId() });
                        c.ztmenu['split_cells'] = m.add({id: DOM.uniqueId(),title : 'table.split_cells_desc', cmd : 'mceTableSplitCells', ui: true, icon: 'split_cells'});
                        c.ztmenu['split_cells'].setDisabled(1);
                        c.ztmenu['merge_cells'] = m.add({id: DOM.uniqueId(),title : 'table.merge_cells_desc', cmd : 'mceTableMergeCells', ui: true, icon: 'merge_cells'});
                        c.ztmenu['merge_cells'].setDisabled(1);
                        m.addSeparator({id : DOM.uniqueId() });
                        c.ztmenu['delete_table'] = m.add({id: DOM.uniqueId()    ,title : 'table.del', cmd : 'mceTableDelete', icon: 'delete_table'});
