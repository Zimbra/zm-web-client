...
ZmMailMsgView.prototype.set =
		function(msg, force) {
			if (!force && this._msg && msg && (this._msg.id == msg.id)) {
				return;
			}
...
ZmMailMsgView.prototype._checkImgInAttachments =
		function(img) {
			if (!this._msg || img.getAttribute("zmforced")) {
				return;
			}
...
ZmMailMsgView.prototype._renderMessage =
		function(msg, container, callback) {
			...
			if (contactsApp) {
				var lookupAddrs = [];
				if (sentBy) {
					lookupAddrs.push(sentBy);
				}
				if (obo) {
					lookupAddrs.push(obo);
				}
				for (var i = 1; i < ZmMailMsg.ADDRS.length; i++) {
					var type = ZmMailMsg.ADDRS[i];
					if (type == AjxEmailAddress.SENDER) {
						continue;
					}
...
ZmMailMsgView.prototype._setTags =
		function(msg) {
			if (!appCtxt.get(ZmSetting.TAGGING_ENABLED) || msg == null || !this._tagList) {
				return;
			}
