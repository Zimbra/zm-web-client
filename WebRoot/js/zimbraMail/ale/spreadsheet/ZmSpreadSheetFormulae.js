...
ZmSpreadSheetFormulae.GOOD_OLD_COMMA = {
	type : ZmSpreadSheetFormulae.TOKEN.COMMA,
	val  : ','
};
...
// 				throw "Consecutive operators not allowed";
	...
	for (var i = 0; i < tokens.length; ++i) {
 		...
 		if (t.isOpr) {
			...
			if (stack.length == 0) // misplaced comma or parens
...
// 		var tmp = stack[i].type.type + " = " + stack[i].val;
// 		if (stack[i].type === TYPE.IDENTIFIER)
// 			tmp += ", n_args: " + stack[i].n_args;
// 		txt.push(tmp);
// 	}
// 	alert(txt.join("\n")/* + "\n\n" + this.eval()*/);
