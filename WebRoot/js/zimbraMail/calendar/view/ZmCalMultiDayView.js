...
function ZmCalMultiDayView(parent, posStyle, dropTgt, view, numDays) {
	...
	ZmCalNewBaseView.call(this, parent, className, posStyle, view, dropTgt);
...
ZmCalMultiDayView._hoursDivWidth_pad = 5; // space between hours div and appts

ZmCalMultiDayView._allDaySepHeight = 4; // height of separator between all day appts and body
ZmCalMultiDayView._daySepWidth = 1; // width of separator between days

ZmCalMultiDayView._minApptWidth = 20;
ZmCalMultiDayView._dayHeadingHeight = 23;
ZmCalMultiDayView._dayFbWidth = 5;
ZmCalMultiDayView._scrollBarWidth = 20;

ZmCalMultiDayView._apptXFudge = 0; // due to border stuff
ZmCalMultiDayView._apptYFudge = -1; // ditto
ZmCalMultiDayView._apptWidthFudge = (AjxEnv.isIE ? 0 : -3); // due to border stuff
ZmCalMultiDayView._apptHeightFudge = (AjxEnv.isIE ? 0 : -4); // ditto


ZmCalMultiDayView._hourHeight = 42;
ZmCalMultiDayView._halfHourHeight = 21;
ZmCalMultiDayView._15minuteHeight = 10.5;
ZmCalMultiDayView._dayHeight = 1008; // 24*42

ZmCalMultiDayView._idToView = {}; // used to find a view object given the id for the appts div

ZmCalMultiDayView.prototype.toString = 
function() {
	return "ZmCalMultiDayView";
}

ZmCalMultiDayView.prototype.getRollField =
function(isDouble) {
	switch(this.view) {
...
ZmCalMultiDayView.prototype._updateRange =
...
ZmCalMultiDayView.prototype._dateUpdate =
function(rangeChanged) {
	this._clearSelectedTime();
	this._updateSelectedTime();
}

ZmCalMultiDayView.prototype._preSet = 
function() {
	this._resetAllDayData();
}

ZmCalMultiDayView.prototype._postSet = 
function() {
//DBG.println("_postSet");
	...
	if (!this._needFirstLayout)
		this._layoutAppts();
	this._layout();
	ZmCalMultiDayView._scrollTo8AM([this]);
}

ZmCalMultiDayView._scrollTo8AM =
function(args)
{
	var view = args[0];
	var bodyElement = Dwt.getDomObj(view.getDocument(), view._bodyDivId);
	bodyElement.scrollTop = ZmCalMultiDayView._hourHeight*8 - 10;
}

ZmCalMultiDayView.prototype._updateTitle =
function() 
{
	if (this._numDays == 1) {
		var date = this._date;	
...
ZmCalMultiDayView.prototype._dayTitle =
function(date) {
...
ZmCalMultiDayView.prototype._updateDays =
function() {
...
ZmCalMultiDayView.prototype._resetAllDayData =
function() {
	this._allDayAppts = {};
 ...
 */
ZmCalMultiDayView.prototype._fanoutAllDay =
function(appt) {
	return false;
}
 ...
 *
 */
ZmCalMultiDayView.prototype._addAllDayAppt = 
function(ao, now) {
...
ZmCalMultiDayView.prototype.addAppt = 
function(ao, now) {
	if (ao.isAllDayEvent()) {
...
ZmCalMultiDayView.prototype._getDivForAppt =
function(appt) {
	return Dwt.getDomObj(this.getDocument(), this._apptBodyDivId);
}

// move this to Dwt?
ZmCalMultiDayView._setOpacity =
function(el, opacity) {
	if (AjxEnv.isIE) el.style.filter = "alpha(opacity="+opacity+")";
	else el.style.opacity = opacity/100;
}

ZmCalMultiDayView._setApptOpacity =
function(appt, div) {
	var pstatus = appt.getParticipationStatus();
	if (pstatus == ZmAppt.PSTATUS_DECLINED) {
		ZmCalMultiDayView._setOpacity(div, 20);
	} else if (pstatus == ZmAppt.PSTATUS_TENTATIVE) {
		ZmCalMultiDayView._setOpacity(div, 60);
	} else {
		ZmCalMultiDayView._setOpacity(div, 100);	
	}
}

ZmCalMultiDayView.prototype._createItemHtml =
...
//		tag: isNew ? "NEW" : "",		//  HACK: i18n
	...
	if (!appt.isReadOnly()) {
		...
		sash.onmousedown = ZmCalMultiDayView._sashMouseDownHdlr;
		sash.onmouseover = ZmCalMultiDayView._sashMouseOverHdlr;
		sash.onmouseout = ZmCalMultiDayView._sashMouseOutHdlr;
...
ZmCalMultiDayView.prototype._createAllDayItemHtml =
function(appt, now, isDndIcon) {
...
ZmCalMultiDayView.prototype._getDayForDate =
function(d) 
{
...
ZmCalMultiDayView.prototype._createHeadersHtml =
function(html,idx) {
...
ZmCalMultiDayView.prototype._createHoursHtml =
function(html,idx) {
	html[idx++] = "<div style='position:absolute; width:"+ZmCalMultiDayView._hoursDivWidth+"px;' id='"+this._bodyHourDivId+"'>";
...
ZmCalMultiDayView.prototype._createHtml =
function(abook) {
...
//	html[idx++] =  "<div id='"+this._apptBodyDivId+"' class='ImgCalendarDayGrid_BG' style='width:100%; height:1008px; position:absolute;' onclick='ZmCalMultiDayView._onclickHandler(event)' ondblclick='ZmCalMultiDayView._ondblclickHandler(event)'>";
...
ZmCalMultiDayView.prototype._computeMaxCols =
function(layout, max) {
 ...
 */
ZmCalMultiDayView.prototype._computeApptLayout =
function() {
 ...
 */
ZmCalMultiDayView.prototype._addAllDayApptRowLayout =
function() {
	var data = [];
	for (var i=0; i < this._numDays; i++) {
 ...
 */
ZmCalMultiDayView.prototype._fillAllDaySlot = 
 ...
 */
ZmCalMultiDayView.prototype._findAllDaySlot = 
 ...
 */
ZmCalMultiDayView.prototype._computeAllDayApptLayout =
function() {
...
ZmCalMultiDayView.prototype._layoutAllDayAppts =
function() {
	var rows = this._allDayApptsRowLayouts;
...
ZmCalMultiDayView._getApptWidthPercent = 
function(numCols) {
	switch(numCols) {
		case 1: return 1;
		case 2: return 0.8;
...
ZmCalMultiDayView.prototype._getYfromDate = 
function(d, isStart) {
	var h = d.getHours();
	var m = d.getMinutes();
	if (!isStart) {
	...
	return Math.floor(h*ZmCalMultiDayView._hourHeight + (m/60)*ZmCalMultiDayView._hourHeight);
}

ZmCalMultiDayView.prototype._layout =
function() {
	DBG.println("ZmCalMultiDayView in layout!");
	var doc = this.getDocument();
	...
	var bodyY = headerHeight + ZmCalMultiDayView._allDaySepHeight;
	...
	var apptsDivX =  ZmCalMultiDayView._hoursDivWidth + ZmCalMultiDayView._hoursDivWidth_pad;
	var apptsDivHeight = ZmCalMultiDayView._dayHeight + 1; // extra for midnight to show up
	...
	for (var i =0; i < this._numDays; i++) {
		...
		Dwt.setLocation(daySepDiv, currentX + apptsDivX, 0);
		Dwt.setSize(daySepDiv, ZmCalMultiDayView._daySepWidth, apptsDivHeight);
		var day = this._days[i];
		day.apptX = currentX + ZmCalMultiDayView._daySepWidth;
...
ZmCalMultiDayView.prototype._controlListener =
function(ev) {
...
ZmCalMultiDayView.prototype._apptSelected =
function() {
	this._clearSelectedTime();
}

ZmCalMultiDayView._ondblclickHandler =
function (ev){
	ev = DwtUiEvent.getEvent(ev);
	ev._isDblClick = true;
	ZmCalMultiDayView._onclickHandler(ev);
};

ZmCalMultiDayView._onclickHandler =
function(event) {
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(event);	

DBG.println("_onclickHandler: element "+mouseEv.elementX+ ", "+mouseEv.elementY);
DBG.println("_onclickHandler:  doc "+mouseEv.docX+ ", "+mouseEv.docY);
//DBG.dumpObj(mouseEv);
	return;
	...
	if (!element) return;
	var id = element.id;
	var view = ZmCalMultiDayView._idToView[id];
...
/*
ZmCalMultiDayView._oncontextmenuHandler =
function(event) {
	var event = DwtUiEvent.getEvent(event);
	var element = DwtUiEvent.getTargetWithProp(event, "id");
	if (!element) return;
	var id = element.id;
	var data = ZmCalMultiDayView._idToData[id];
	var view = data.view;
	if (view) {
...
ZmCalMultiDayView._sashMouseOverHdlr =
function(ev) {
//	DBG.println("ZmCalMultiDayView._sashMouseOverHdlr");
	var mouseEv = DwtShell.mouseEvent;
...
ZmCalMultiDayView._sashMouseDownHdlr =
function(event) {
//	DBG.println("ZmCalMultiDayView._sashMouseDownHdlr");
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(event);	
...
//	DBG.println("origHeight = "+origHeight);
	...
	var capture = new DwtMouseEventCapture	(data,
			ZmCalMultiDayView._sashMouseOverHdlr,
			ZmCalMultiDayView._sashMouseDownHdlr, ZmCalMultiDayView._sashMouseMoveHdlr, 
			ZmCalMultiDayView._sashMouseUpHdlr, ZmCalMultiDayView._sashMouseOutHdlr, true);
	capture.capture();
	mouseEv._stopPropagation = true;
	mouseEv._returnValue = false;
...
ZmCalMultiDayView._getAttrFromElement =
function(el, attr)  {
	while (el != null) {
		if (el.getAttribute) {
...
//	DBG.println("ZmCalMultiDayView._sashMouseMoveHdlr");
	...
	var delta15 = Math.floor(delta/ZmCalMultiDayView._15minuteHeight);
	delta = delta15 * ZmCalMultiDayView._15minuteHeight;

	if (delta != data.lastDelta) {
		var newHeight = data.origHeight + delta;
		if (newHeight >= ZmCalMultiDayView._15minuteHeight) {
//			DBG.println("delta = " + delta);
			Dwt.setSize(data.apptBodyEl, Dwt.DEFAULT, newHeight + ZmCalMultiDayView._apptHeightFudge);
...
ZmCalMultiDayView._sashMouseUpHdlr =
function(ev) {
//	DBG.println("ZmCalMultiDayView._sashMouseUpHdlr");
	var data = DwtMouseEventCapture.getTargetObj();
	ZmCalMultiDayView._setApptOpacity(data.appt, data.apptEl);	
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(ev);	
	if (mouseEv.button != DwtMouseEvent.LEFT) {
...
ZmCalMultiDayView._sashMouseOutHdlr =
function(ev) {
//	DBG.println("ZmCalMultiDayView._sashMouseOutHdlr");
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(ev);
	mouseEv._stopPropagation = true;
	mouseEv._returnValue = false;
