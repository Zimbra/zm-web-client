...
function ZmCalScheduleView(parent, posStyle, dropTgt) {
	...
	var view = ZmController.CAL_DAY_VIEW;
...
ZmCalScheduleView.DRAG_THRESHOLD = 5;

ZmCalScheduleView._OPACITY_APPT_NORMAL = 100;
ZmCalScheduleView._OPACITY_APPT_DECLINED = 20;
ZmCalScheduleView._OPACITY_APPT_TENTATIVE = 60;
ZmCalScheduleView._OPACITY_APPT_DND = 70;

ZmCalScheduleView._HOURS_DIV_WIDTH = 40; // width of div holding hours text (1:00am, etc)
ZmCalScheduleView._UNION_DIV_WIDTH = 40; // width of div holding union in sched view
ZmCalScheduleView._HOURS_DIV_WIDTH_PAD = 0; // space between hours div and appts

ZmCalScheduleView._ALL_DAY_SEP_HEIGHT = 5; // height of separator between all day appts and body
ZmCalScheduleView._DAY_SEP_WIDTH = 2; // width of separator between days

ZmCalScheduleView._SCROLLBAR_WIDTH = 20;

ZmCalScheduleView._DAY_HEADING_HEIGHT = 20;
ZmCalScheduleView._ALL_DAY_APPT_HEIGHT = 20;
ZmCalScheduleView._ALL_DAY_APPT_HEIGHT_PAD = 3; // space between all day appt rows
ZmCalScheduleView._APPT_X_FUDGE = 0; // due to border stuff
ZmCalScheduleView._APPT_Y_FUDGE = -1; // ditto
ZmCalScheduleView._APPT_WIDTH_FUDGE = (AjxEnv.isIE ? 0 : -3); // due to border stuff
ZmCalScheduleView._APPT_HEIGHT_FUDGE = (AjxEnv.isIE ? 0 : -4); // ditto

ZmCalScheduleView._HOUR_HEIGHT = 42;
ZmCalScheduleView._15_MINUTE_HEIGHT = ZmCalScheduleView._HOUR_HEIGHT/4;
ZmCalScheduleView._DAY_HEIGHT = ZmCalScheduleView._HOUR_HEIGHT*24;
...
ZmCalScheduleView.prototype.getRollField =
function(isDouble) {
	switch(this.view) {
...
ZmCalScheduleView.prototype.getPrintHtml = 
function() {
	var html = new Array();
	var idx = 0;
...
ZmCalScheduleView.prototype._loadDetailsForAppts = 
...
ZmCalScheduleView.prototype._printApptDetails = 
function(appt) {
	var html = new Array();
	var idx = 0;
...
ZmCalScheduleView.prototype._getDateHdrForPrintView = 
function() {
	var header = "";
	var timeRange = this.getTimeRange();
	var startDate = new Date(timeRange.start);
...
ZmCalScheduleView.prototype._selectDay =
function(date) {
...
ZmCalScheduleView.prototype._clearSelectedTime =
function() 
{
...
ZmCalScheduleView.prototype._updateSelectedTime =
function() 
{
	...
	ZmCalScheduleView._setOpacity(e, 40);
	Dwt.setVisible(e, true);
}

ZmCalScheduleView.prototype._removeNode =
function(doc, id) {
...
function() {
	...
	if (this._numCalendars > 0) {
		for (var i =0; i < this._numCalendars; i++) {
			...
			this._removeNode(doc, cal.titleId);
			this._removeNode(doc, cal.headingDaySepDivId);
			this._removeNode(doc, cal.daySepDivId);			
		}
	}

	this._calendars = this._calController.getCheckedCalendars();
	this._calendars.sort(ZmCalendar.sortCompare);
	this._folderIdToCalIndex = {};
	this._cals = [];
	...
	for (var i =0; i < this._numCalendars; i++) {
		...
		this._folderIdToCalIndex[this._calendars[i].id] = cal;

		//html.append("<div id='", this._days[i].titleId, "' class=calendar_heading_day style='position:absolute;'>
		// <div class=calendar_heading_day_text></div></div>");
		...
		div.style.position = 'absolute';		
		div.id = cal.headingDaySepDivId;
		headingParentEl.appendChild(div);						

		//SV	  html.append("<div id='", this._days[i].daySepDivId, "' class='calendar_day_separator' style='position:absolute'></div>");
		div = doc.createElement("div");
		div.className = "calendar_day_separator";
		div.style.position = 'absolute';		
		div.id = cal.daySepDivId;
		dayParentEl.appendChild(div);						
	}
}

ZmCalScheduleView.prototype._layoutCalendars =
function() {

}

ZmCalScheduleView.prototype._preSet = 
function() {
	this._resetCalendarData(); // cal must be first
	this._resetAllDayData();
}

ZmCalScheduleView.prototype._postSet = 
function() {
DBG.println("sched view _postSet");
	this._layoutCalendars();	
...
ZmCalScheduleView.prototype._syncScroll =
function()
{
...
ZmCalScheduleView.prototype._scrollTo8AM =
function()
{
	if (!this._autoScrollDisabled) {
		var bodyElement = Dwt.getDomObj(this.getDocument(), this._bodyDivId);
		bodyElement.scrollTop = ZmCalScheduleView._HOUR_HEIGHT*8 - 10;
		this._syncScroll();
	} else {
...
ZmCalScheduleView.prototype._updateTitle =
function() 
{
	var numDays = this.getNumDays();
	if (numDays == 1) {
...
ZmCalScheduleView.prototype._dayTitle =
function(date) {
...
ZmCalScheduleView.prototype._updateDays =
function() {
...
ZmCalScheduleView.prototype._resetAllDayData =
function() {
	this._allDayAppts = {};
 ...
 */
ZmCalScheduleView.prototype._fanoutAllDay =
function(appt) {
	return false;
}

ZmCalScheduleView.prototype._getDivForAppt =
function(appt) {
	return Dwt.getDomObj(this.getDocument(), appt.isAllDayEvent() ? this._allDayDivId : this._apptBodyDivId);		 
}

// move this to Dwt?
ZmCalScheduleView._setOpacity =
function(el, opacity) {
	if (AjxEnv.isIE) el.style.filter = "alpha(opacity="+opacity+")";
	else el.style.opacity = opacity/100;
}

ZmCalScheduleView._setApptOpacity =
function(appt, div) {
	var pstatus = appt.getParticipationStatus();
	if (pstatus == ZmAppt.PSTATUS_DECLINED) {
		ZmCalScheduleView._setOpacity(div, ZmCalScheduleView._OPACITY_APPT_DECLINED);
	} else if (pstatus == ZmAppt.PSTATUS_TENTATIVE) {
		ZmCalScheduleView._setOpacity(div, ZmCalScheduleView._OPACITY_APPT_TENTATIVE);
	} else {
		ZmCalScheduleView._setOpacity(div, ZmCalScheduleView._OPACITY_APPT_NORMAL);	
	}
}
...
function(div, allDay, folderId) {
	...
	ZmCalScheduleView._setOpacity(div, ZmCalScheduleView._OPACITY_APPT_DND);
	var subs = {
		id: div.id,
		newState: "",
...
ZmCalScheduleView.prototype._createItemHtml =
function(appt) {
	...
	ZmCalScheduleView._setApptOpacity(appt, div);
...
ZmCalScheduleView.prototype._createHoursHtml =
function(html) {
	html.append("<div style='position:absolute; top:-8; width:", ZmCalScheduleView._HOURS_DIV_WIDTH, "px;' id='", this._bodyHourDivId, "'>");
	...
	for (var h=0; h < 25; h++) {
		...
		ZmCalScheduleView._HOUR_HEIGHT ,"px; width:", ZmCalScheduleView._HOURS_DIV_WIDTH, "px'><div class=calendar_grid_body_time_text>");
		if (h == 0 || h == 24) {
			html.append("&nbsp;");
		} else if (h == 12) {
...
function(abook) {
	...
	for (var i =0; i < numDays; i++) {
		this._days[i] = {
			index: i,
//SV			titleId: Dwt.getNextId(),
//SV			headingDaySepDivId: Dwt.getNextId(),
//SV			daySepDivId: Dwt.getNextId(),
			apptX: 0, // computed in layout
			apptWidth: 0,// computed in layout
			allDayX: 0, // computed in layout
			allDayWidth: 0// computed in layout			
	...
	html.append("<div id='", this._headerYearId, 
		"' class=calendar_heading_year_text style='position:absolute; width:", ZmCalScheduleView._HOURS_DIV_WIDTH,"px;'></div>");
	html.append("</div>");
	...
	if (this._scheduleMode) {
		...
		html.append("<div class=calendar_heading_year_text style='position:absolute; width:", ZmCalScheduleView._HOURS_DIV_WIDTH,"px;'>",ZmMsg.all,"</div>");
		html.append("</div>");
...
//SV	for (var i =0; i < this.getNumDays(); i++) {
//SV		html.append("<div id='", this._days[i].titleId, "' class=calendar_heading_day style='position:absolute;'><div class=calendar_heading_day_text></div></div>");
//SV	}
	html.append("</div>");
	
	// divs to separate day headings
//SV	for (var i =0; i < numDays; i++) {
//SV		html.append("<div id='", this._days[i].headingDaySepDivId, "' class='calendar_day_separator' style='position:absolute'></div>");
//SV	}
...
ZmCalScheduleView.prototype._computeMaxCols =
function(layout, max) {
 ...
 */
ZmCalScheduleView.prototype._computeApptLayout =
function() {
 ...
 */
ZmCalScheduleView.prototype._addAllDayApptRowLayout =
function() {
	var data = [];
 ...
 */
ZmCalScheduleView.prototype._fillAllDaySlot = 
 ...
 */
ZmCalScheduleView.prototype._findAllDaySlot = 
 ...
 */
ZmCalScheduleView.prototype._computeAllDayApptLayout =
function() {
	...
	for (var i=0; i < adlist.length; i++) {
		...
		if (data) {
			if (this._scheduleMode) {
				var cal = this._getCalForFolderId(data.appt.getFolderId());
				if (cal)	 this._findAllDaySlot(cal.index, data);			
			} else {
...
function() {
	...
	var doc = this.getDocument();
	var rowY = ZmCalScheduleView._ALL_DAY_APPT_HEIGHT_PAD + 2;
	for (var i=0; i < rows.length; i++) {
		var row = rows[i];
		var num = this._scheduleMode ? this._numCalendars : this.getNumDays();
		for (var j=0; j < num; j++) {
			...
			if (slot.data) {
				...
				if (this._scheduleMode) {
					var cal= this._getCalForFolderId(appt.getFolderId());
					this._positionAppt(div, cal.allDayX+0, rowY);
					this._sizeAppt(div, cal.allDayWidth * slot.data.numDays - ZmCalScheduleView._DAY_SEP_WIDTH - 1,
								 ZmCalScheduleView._ALL_DAY_APPT_HEIGHT);
				 } else {
					this._positionAppt(div, this._days[j].allDayX+0, rowY);
					this._sizeAppt(div, this._days[j].allDayWidth * slot.data.numDays - ZmCalScheduleView._DAY_SEP_WIDTH - 1,
								 ZmCalScheduleView._ALL_DAY_APPT_HEIGHT);
				 }
				 
			}
		}
		rowY += ZmCalScheduleView._ALL_DAY_APPT_HEIGHT + ZmCalScheduleView._ALL_DAY_APPT_HEIGHT_PAD;
	}
}

ZmCalScheduleView._getApptWidthPercent = 
function(numCols) {
	switch(numCols) {
		case 1: return 1;
		case 2: return 0.8;
...
ZmCalScheduleView.prototype._positionAppt =
function(apptDiv, x, y) {
	// position overall div
	Dwt.setLocation(apptDiv, x + ZmCalScheduleView._APPT_X_FUDGE, y + ZmCalScheduleView._APPT_Y_FUDGE);
}
...
ZmCalScheduleView.prototype._layoutAppts =
function() {
	...
	for (var i=0; i < this._layouts.length; i++) {
		...
		if (apptDiv) {
			...
			var w = Math.floor(layout.bounds.width*ZmCalScheduleView._getApptWidthPercent(layout.maxcol+1));
...
ZmCalScheduleView.prototype._getDayForDate =
function(d) 
{
	return this._dateToDayIndex[this._dayKey(d)];
}	

ZmCalScheduleView.prototype._getCalForFolderId =
function(folderId) 
{
	return this._folderIdToCalIndex[folderId];
}	

ZmCalScheduleView.prototype._getDayFromX =
function(x) {
	var numDays = this.getNumDays();
	for (var i =0; i < numDays; i++) {
		var day = this._days[i];
...
function(d) {
	...
	return new DwtPoint(day.apptX, Math.floor(((h+m/60) * ZmCalScheduleView._HOUR_HEIGHT))+1);
}

ZmCalScheduleView.prototype._getBoundsForAppt =
function(appt) {
...
function(d, duration, folderId) {
	...
	var h = d.getHours();
	var m = d.getMinutes();
	var cal= this._getCalForFolderId(folderId);
	if (cal == null) return null;
	return new DwtRectangle(cal.apptX, ((h+m/60) * ZmCalScheduleView._HOUR_HEIGHT), 
					cal.apptWidth, (ZmCalScheduleView._HOUR_HEIGHT / 60) * durationMinutes);
}
...
function(x, y, snapMinutes, roundUp) {
	...
	if (day == null) return null;
	var minutes = Math.floor((y / ZmCalScheduleView._HOUR_HEIGHT) * 60);
...
ZmCalScheduleView.prototype._getAllDayDateFromXY =
function(x, y) {
	var day = this._getDayFromX(x);
	if (day == null) return null;
...
ZmCalScheduleView.prototype._setBounds =
function(id, x, y, w, h) {
	var el = Dwt.getDomObj(this.getDocument(), id);
	if (el == null) {
		DBG.println("ZmCalScheduleView._setBounds null element for id: "+id);
	} else {
...
function() {
	DBG.println("ZmCalScheduleView in layout!");
	var doc = this.getDocument();
	...
	var hoursWidth = ZmCalScheduleView._HOURS_DIV_WIDTH;
	
	var bodyX = hoursWidth + ZmCalScheduleView._DAY_SEP_WIDTH;
	var unionX = bodyX;
	if (this._scheduleMode) {
		bodyX += ZmCalScheduleView._UNION_DIV_WIDTH + ZmCalScheduleView._DAY_SEP_WIDTH;
	}
	...
	this._allDayDivHeight = (ZmCalScheduleView._ALL_DAY_APPT_HEIGHT+ZmCalScheduleView._ALL_DAY_APPT_HEIGHT_PAD) * numRows + ZmCalScheduleView._ALL_DAY_APPT_HEIGHT_PAD;
	...
	this._setBounds(this._leftAllDaySepDivId, hoursWidth, 0, ZmCalScheduleView._DAY_SEP_WIDTH, allDayScrollHeight);		
	
	// horiz separator between all day appts and grid
	this._setBounds(this._allDaySepDivId, 0, allDayScrollHeight, width, ZmCalScheduleView._ALL_DAY_SEP_HEIGHT);	

	// compute height for hours/grid
	this._bodyDivWidth = width - bodyX;
	
	var bodyY = allDayScrollHeight + ZmCalScheduleView._ALL_DAY_SEP_HEIGHT +  (AjxEnv.isIE ? 0 : 2);
	...
	this._setBounds(this._leftApptSepDivId, hoursWidth, bodyY, ZmCalScheduleView._DAY_SEP_WIDTH, ZmCalScheduleView._DAY_HEIGHT);	

	// div for scrolling grid	
	this._setBounds(this._bodyDivId, bodyX, bodyY, this._bodyDivWidth, this._bodyDivHeight);	

	// size appts divs
	var apptsDivX =  ZmCalScheduleView._HOURS_DIV_WIDTH_PAD;
	this._apptBodyDivHeight = ZmCalScheduleView._DAY_HEIGHT + 1; // extra for midnight to show up
	...
	if (this._scheduleMode) {
		//heading
		this._setBounds(this._unionHeadingDivId, unionX, 0, ZmCalScheduleView._UNION_DIV_WIDTH, Dwt.DEFAULT);
				
		//div under heading
		this._setBounds(this._unionAllDayDivId, unionX, allDayDivY, ZmCalScheduleView._UNION_DIV_WIDTH, this._allDayDivHeight);		

		// sep in all day area
		var unionSepX = unionX + ZmCalScheduleView._UNION_DIV_WIDTH;
		this._setBounds(this._unionHeadingSepDivId, unionSepX, 0, ZmCalScheduleView._DAY_SEP_WIDTH, allDayScrollHeight);		

		// div for scrolling union
		this._setBounds(this._unionGridScrollDivId, unionX, bodyY, ZmCalScheduleView._UNION_DIV_WIDTH, this._bodyDivHeight);		
		this._setBounds(this._unionGridDivId, 0, -1, ZmCalScheduleView._UNION_DIV_WIDTH, this._apptBodyDivHeight+ZmCalScheduleView._HOUR_HEIGHT);		

		// sep in grid area
		this._setBounds(this._unionGridSepDivId, unionSepX, bodyY, ZmCalScheduleView._DAY_SEP_WIDTH, this._apptBodyDivHeight);		
	}

	var numDays = this.getNumDays();
	var dayWidth = Math.floor((this._apptBodyDivWidth-ZmCalScheduleView._SCROLLBAR_WIDTH)/numDays);
...
//		this._setBounds(day.titleId, apptsDivX+currentX+1, Dwt.DEFAULT, dayWidth-2 - dayWfudge, ZmCalScheduleView._DAY_HEADING_HEIGHT-2);

		day.apptX = currentX + 2 ; //ZZZ
		day.apptWidth = dayWidth - ZmCalScheduleView._DAY_SEP_WIDTH - 3;  //ZZZZ
		day.allDayX = apptsDivX + day.apptX;
		day.allDayWidth = dayWidth; // doesn't include sep
		currentX += dayWidth;

//		this._setBounds(day.headingDaySepDivId, apptsDivX+currentX, 0, ZmCalScheduleView._DAY_SEP_WIDTH, allDayHeadingDivHeight + this._allDayDivHeight);
//		this._setBounds(day.daySepDivId, currentX, 0, ZmCalScheduleView._DAY_SEP_WIDTH, this._apptBodyDivHeight);

		currentX += ZmCalScheduleView._DAY_SEP_WIDTH;
	}	
	//SZ

	currentX = 0;
	var dayWidth = Math.floor((this._apptBodyDivWidth-ZmCalScheduleView._SCROLLBAR_WIDTH)/this._numCalendars);
	...
	for (var i = 0; i < this._numCalendars; i++) {
		...
		// position day heading
		var dayWfudge = (day.isToday && !AjxEnv.isIE) ? 1 : 0;
		this._setBounds(cal.titleId, apptsDivX+currentX+1, Dwt.DEFAULT, dayWidth-2 - dayWfudge, ZmCalScheduleView._DAY_HEADING_HEIGHT-2);

		cal.apptX = currentX + 2 ; //ZZZ
		cal.apptWidth = dayWidth - ZmCalScheduleView._DAY_SEP_WIDTH - 3;  //ZZZZ
		cal.allDayX = apptsDivX + cal.apptX;
		cal.allDayWidth = dayWidth; // doesn't include sep
		currentX += dayWidth;

		this._setBounds(cal.headingDaySepDivId, apptsDivX+currentX, 0, ZmCalScheduleView._DAY_SEP_WIDTH, allDayHeadingDivHeight + this._allDayDivHeight);
		this._setBounds(cal.daySepDivId, currentX, 0, ZmCalScheduleView._DAY_SEP_WIDTH, this._apptBodyDivHeight);
		currentX += ZmCalScheduleView._DAY_SEP_WIDTH;
	}	
...
ZmCalScheduleView.prototype._handleApptScrollRegion =
...
ZmCalScheduleView.prototype._controlListener =
function(ev) {
...
ZmCalScheduleView.prototype._apptSelected =
function() {
	//
}

ZmCalScheduleView._ondblclickHandler =
function (ev){
	ev = DwtUiEvent.getEvent(ev);
	ev._isDblClick = true;
	ZmCalScheduleView._onclickHandler(ev);
};

ZmCalScheduleView.prototype._mouseOverAction =
function(ev, div) {
...
ZmCalScheduleView.prototype._mouseOutAction =
function(ev, div) {
...
ZmCalScheduleView.prototype._timeSelectionAction =
...
function(ev, apptEl) {
	...
	var capture = new DwtMouseEventCapture	(data,
			ZmCalScheduleView._emptyHdlr, // mouse over
			ZmCalScheduleView._emptyHdlr, // mouse down (already handled by action)
			ZmCalScheduleView._apptMouseMoveHdlr, 
			ZmCalScheduleView._apptMouseUpHdlr, 
			ZmCalScheduleView._emptyHdlr, // mouse out
			true);
	capture.capture();
	return false;	
}
...
ZmCalScheduleView.prototype._apptDndBegin =
function(data) {
	...
	ZmCalScheduleView._setOpacity(data.apptEl, ZmCalScheduleView._OPACITY_APPT_DND);
...
function(ev) {
	...
	if (!data.dndStarted) {
		var withinThreshold =  (Math.abs(deltaX) < ZmCalScheduleView.DRAG_THRESHOLD && Math.abs(deltaY) < ZmCalScheduleView.DRAG_THRESHOLD);
	...
	var scrollOffset = data.view._handleApptScrollRegion(mouseEv.docX, mouseEv.docY, ZmCalScheduleView._HOUR_HEIGHT);
	if (scrollOffset != 0) {
...
ZmCalScheduleView._apptMouseUpHdlr =
function(ev) {
//	DBG.println("ZmCalScheduleView._sashMouseUpHdlr");
	var data = DwtMouseEventCapture.getTargetObj();
	ZmCalScheduleView._setApptOpacity(data.appt, data.apptEl);	
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(ev);	
	...
	if (data.dndStarted) {
		...
		if (data.startDate.getTime() != data.appt.getStartTime()) {
			...
			var errorCallback = new AjxCallback(null, ZmCalScheduleView._handleError, data);
			cc.updateApptDate(data.appt._orig, data.startDate, endDate, false, null, errorCallback);
		} else {
			ZmCalScheduleView._restoreLayout(data);
		}
	}
...
ZmCalScheduleView.prototype._sashMouseDownAction =
function(ev, sash) {
//	DBG.println("ZmCalScheduleView._sashMouseDownHdlr");
	if (ev.button != DwtMouseEvent.LEFT) {
		return false;
	}
	...
	var capture = new DwtMouseEventCapture	(data,
			ZmCalScheduleView._emptyHdlr, // mouse over
			ZmCalScheduleView._emptyHdlr, // mouse down (already handled by action)
			ZmCalScheduleView._sashMouseMoveHdlr, 
			ZmCalScheduleView._sashMouseUpHdlr, 
			ZmCalScheduleView._emptyHdlr, // mouse out
			true);
	capture.capture();
	this.deselectAll();
	this.setSelection(data.appt);
	ZmCalScheduleView._setOpacity(apptEl, ZmCalScheduleView._OPACITY_APPT_DND);
	return false;	
}

ZmCalScheduleView._sashMouseMoveHdlr =
function(ev) {
//	DBG.println("ZmCalScheduleView._sashMouseMoveHdlr");
	var mouseEv = DwtShell.mouseEvent;
	mouseEv.setFromDhtmlEvent(ev);	
	var delta = 0;
	var data = DwtMouseEventCapture.getTargetObj();
	...
	var delta15 = Math.floor(delta/ZmCalScheduleView._15_MINUTE_HEIGHT);
	delta = delta15 * ZmCalScheduleView._15_MINUTE_HEIGHT;

	if (delta != data.lastDelta) {
		if (data.isTop) {
			var newY = data.apptY + delta;
			var newHeight = data.origHeight - delta;
			if (newHeight >= ZmCalScheduleView._15_MINUTE_HEIGHT) {
...
//			DBG.println("delta = " + delta);
				...
				Dwt.setSize(data.apptBodyEl, Dwt.DEFAULT, newHeight + ZmCalScheduleView._APPT_HEIGHT_FUDGE);
...
ZmCalScheduleView.prototype._gridMouseDownAction =
function(ev, gridEl, gridLoc, isAllDay) {
	...
	var capture = new DwtMouseEventCapture	(data,
			ZmCalScheduleView._emptyHdlr, // mouse over
			ZmCalScheduleView._emptyHdlr, // mouse down (already handled by action)
			isAllDay? ZmCalScheduleView._gridAllDayMouseMoveHdlr : ZmCalScheduleView._gridMouseMoveHdlr,
			ZmCalScheduleView._gridMouseUpHdlr, 
			ZmCalScheduleView._emptyHdlr, // mouse out
			true);
	capture.capture();
	return false;	
...
ZmCalScheduleView._gridMouseMoveHdlr =
function(ev) {
...
ZmCalScheduleView._gridAllDayMouseMoveHdlr =
function(ev) {
...
ZmCalScheduleView._handleError =
function(args) {
	var data	= args.shift();
	var ex		= args.shift();
...
ZmCalScheduleView._restoreLayout =
function(data) {
	var lo = data.layout;
