 ...
 */
ZmAppCtxt.prototype.getTwoFactorAuthMethodAllowed =
function() {
    var defaultOrder = ["app", "email"];

    // "app" is for backward compatibility
    var allowedMethod = appCtxt.get(ZmSetting.TWO_FACTOR_AUTH_METHOD_ALLOWED) || "app";
    if (typeof allowedMethod === "string") {
        // only a single method
        return [allowedMethod];
    } else {
        // note: do not update allowedMethod directly. It will update the original array object returned by appCtxt.get(ZmSetting.TWO_FACTOR_AUTH_METHOD_ALLOWED).
        var allowedMethodSorted = [];
        for (var i = 0; i < defaultOrder.length; i++) {
            if (allowedMethod.indexOf(defaultOrder[i]) !== -1) {
                allowedMethodSorted.push(defaultOrder[i]);
            }
        }
 ...
 */
ZmAppCtxt.prototype.getTwoFactorAuthMethodAllowedAndEnabled =
function() {
    // it has already been sorted
    var allowedMethod = this.getTwoFactorAuthMethodAllowed();

    var enabledMethod = this.get(ZmSetting.TWO_FACTOR_AUTH_METHOD_ENABLED) || [];
    if (typeof enabledMethod === "string") {
        enabledMethod = [enabledMethod];
    }

    // for backward compatibility
    var tfaEnabled = this.get(ZmSetting.TWO_FACTOR_AUTH_ENABLED);
    if (tfaEnabled && enabledMethod.length === 0 && allowedMethod.indexOf("app") !== -1) {
        enabledMethod = ["app"];
    }
...
ZmAppCtxt.prototype.getPrefPrimaryTwoFactorAuthMethod =
function() {
    var enabledMethod = this.getTwoFactorAuthMethodAllowedAndEnabled();
    var primaryMethod = this.get(ZmSetting.TWO_FACTOR_AUTH_PRIMARY_METHOD);
