...
<c:if test="${not empty text}">
    <td height="100%" <c:if test="${not empty width}">width="${width}"</c:if> nowrap valign="middle" style="padding: 0 2px 0 2px">
        <input ${extra} <c:if test="${not empty id}">id="S${id}"</c:if> <c:if test="${disabled}">disabled class='ImgDisabled' </c:if> <c:if test="${not empty clazz}">class="${clazz}"</c:if>  name="${name}" type="submit" value="${fn:escapeXml(text)}"  <c:if test="${not empty tooltip}">title="${fn:escapeXml(tooltip)}"</c:if>>
    </td>
</c:if>
