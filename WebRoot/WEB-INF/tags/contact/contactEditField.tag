...
<tr>
    <td <c:if test="${address}">valign='top'</c:if> class="editContactLabel">${fn:escapeXml(label)}:</td>
    <td>
    <c:choose>
        <c:when test="${address}">
            <textarea name='${field}' cols=32 rows='2'>${fn:escapeXml(value)}</textarea>
        </c:when>
        <c:otherwise>
            <input name='${field}' type='text' autocomplete='off' size='35' value="${fn:escapeXml(value)}">
        </c:otherwise>
    </c:choose>
    </td>
</tr>
