...
<tr>
    <td nowrap colspan="3" class='Folder<c:if test="${folder.hasUnread and types ne 'contact'}"> Unread</c:if>'
        ...
        <a href='${fn:escapeXml(url)}' id="FLDR${folder.id}">
            ...
            <span title="${fn:escapeXml(label)}" <c:if test="${folder.id eq requestScope.context.selectedId}"> class='ZhTISelected'</c:if>>${zm:truncate(fn:escapeXml(label),15,true)}<c:if test="${folder.hasUnread and types ne 'contact'}">
