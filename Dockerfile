...
RUN cd /opt/zimbra/jetty_base/webapps/zimbra/public && cat login.jsp | sed -e '/\/\/ check if maibox is upgraded/,/%>/c\%>' | sed -e 's/value="${isUpgradedMailbox}"/value="true"/' > login.jsp.tmp
